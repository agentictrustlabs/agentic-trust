...
export function parseEnsDid(raw: string | undefined | null): ParsedEnsDid {
  ...
  try {
    ...
    throw new Error(
      `Invalid percent-encoding in ENS DID: ${(error as Error).message}`,
    );
  }

  if (!decoded.startsWith(ENS_DID_PREFIX)) {
    throw new Error(
      `Invalid ENS DID format: ${decoded}. Expected format: did:ens:chainId:ensname`,
    );
  }
  ...
  if (parts.length < 4) {
    throw new Error(
      `ENS DID missing components: ${decoded}. Expected format: did:ens:chainId:ensname`,
    );
  }
  ...
  if (!ensNamePart.toLowerCase().endsWith('.eth')) {
    throw new Error(
      `Invalid ENS name in ENS DID: ${decoded}. ENS name must end with .eth`,
    );
  }
 ...
 */
export function buildEnsDid(
  chainId: number | string,
  ensName: string,
  options?: BuildEnsDidOptions,
): string {
  const chainIdStr =
  ...
  if (!lowerEnsName.endsWith('.eth')) {
    throw new Error(
      `Invalid ENS name: ${normalizedEnsName}. ENS name must end with .eth to be a valid did:ens`,
    );
  }
